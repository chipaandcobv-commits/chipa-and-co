  ...
  try {
    ...
    if (rewardClaims.length > 0) {
      const rewardsSummary = rewardClaimsData.map(claim => ({
        ...
        'Fecha de Actualización': new Date(claim.updatedAt).toLocaleDateString('es-AR'),
        'Fecha de Vencimiento': new Date(claim.expiresAt).toLocaleDateString('es-AR'),
      }));

      const rewardsSheet = XLSX.utils.json_to_sheet(rewardsSummary);
      XLSX.utils.book_append_sheet(workbook, rewardsSheet, 'Premios Canjeados');
    }
    ...
    const totalPointsSpent = rewardClaims.reduce((sum, claim) => sum + claim.pointsSpent, 0);
    ...
    const statsData = [
      ...
      { 'Métrica': 'Puntos Totales Gastados', 'Valor': totalPointsSpent.toLocaleString() },
    ...
    const response = new NextResponse(excelBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="backup-completo-${new Date().toISOString().split('T')[0]}.xlsx"`,
